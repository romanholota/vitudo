...
def index(request):
	if request.GET.get('category'):
		category = get_object_or_404(Category, id=request.GET.get('category'), account=request.user.details.account)
		products = Product.objects.this_account(request.user.details.account).this_category(category)
	else:
		products = Product.objects.this_account(request.user.details.account)

	categories = Category.objects.this_account(request.user.details.account).order_by('name')
...
def add(request):
	form = ProductForm(request.POST or None, account=request.user.details.account)
	...
	if request.POST:
		if form.is_valid() and details_form.is_valid():
			...
			new_product = Product.objects.update_or_create_product(**form.cleaned_data, id=None, details=new_details, account=request.user.details.account, full_name=form.cleaned_data['brand'].name + " " + form.cleaned_data['model'])
...
def items(request, product_id):
	product = get_object_or_404(Product, id=product_id, account=request.user.details.account)
	items = Item.objects.active().this_product(product).this_account(request.user.details.account)
...
def remove(request, product_id):
	product = get_object_or_404(Product, id=product_id, account=request.user.details.account)
	item_count = Item.objects.active().this_product(product).this_account(request.user.details.account).count()
...
def detail(request, product_id):
	product = get_object_or_404(Product, id=product_id, account=request.user.details.account)
	form = ProductForm(request.POST or None, instance=product, account=request.user.details.account)
	details_form = ProductDetailsForm(request.POST or None, request.FILES or None, instance=product.details)

	if request.POST:
		if form.is_valid() and details_form.is_valid():
			new_details = details_form.save()
			product = Product.objects.update_or_create_product(**form.cleaned_data, id=product.id, details=new_details, account=request.user.details.account, full_name=form.cleaned_data['brand'].name + " " + form.cleaned_data['model'])
...
def brands(request):
	brands = Brand.objects.this_account(request.user.details.account).has_products(request.GET.get('hasproducts'))
	form = SearchForm(request.GET or None)
...
def brand_add(request):
	...
	if request.POST and form.is_valid():
		new_brand = Brand.objects.create_brand(**form.cleaned_data, account=request.user.details.account)
...
def brand_products(request, brand_id):
	brand = get_object_or_404(Brand, id=brand_id, account=request.user.details.account)
	products = Product.objects.this_brand(brand).this_account(request.user.details.account)
	form = SearchForm(request.GET or None)
...
def brand_remove(request, brand_id):
	brand = get_object_or_404(Brand, id=brand_id, account=request.user.details.account)
	product_count = Product.objects.this_brand(brand).this_account(request.user.details.account).count()
...
def categories(request):
	...
	categories = Category.objects.this_account(request.user.details.account).has_products(has_products)
...
def category_add(request):
	...
	if request.POST and form.is_valid():
		new_category = Category.objects.create_category(**form.cleaned_data, account=request.user.details.account)
		return redirect(reverse('products:categories'))
	return render(request, 'products/categories/list/new.html', {'form': form})

def category_detail(request, category_id):
	category = get_object_or_404(Category, id=category_id, account=request.user.details.account)
...
def category_products(request, category_id):
	category = get_object_or_404(Category, id=category_id, account=request.user.details.account)
	products = Product.objects.this_category(category).this_account(request.user.details.account)
	form = SearchForm(request.GET or None)
...
def category_remove(request, category_id):
	category = get_object_or_404(Category, id=category_id, account=request.user.details.account)
	product_count = Product.objects.this_category(category).this_account(request.user.details.account).count()
