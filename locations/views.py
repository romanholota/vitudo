...
def index(request):
	locations = Location.objects.this_account(request.user.details.account).has_items(request.GET.get('hasitems'))
...
def add(request):
	form = LocationForm(request.POST or None, account=request.user.details.account)

	if request.POST and form.is_valid():
		new_location = Location.objects.create_location(**form.cleaned_data, account=request.user.details.account)
...
def items(request, location_id):
	...
	items = Item.objects.active().at_location(location).this_account(request.user.details.account)
	form = SearchForm(request.GET or None)
...
def transfers(request, location_id):
	location = get_object_or_404(Location, id=location_id, account=request.user.details.account)
	transfers = Transfer.objects.this_location(location).this_account(request.user.details.account).is_order(request.GET.get('order'))
	form = SearchForm(request.GET or None)
...
def detail(request, location_id):
	location = get_object_or_404(Location, id=location_id, account=request.user.details.account)
	form = LocationForm(request.POST or None, instance=location, account=request.user.details.account)
...
def location_remove(request, location_id):
	location = get_object_or_404(Location, id=location_id, account=request.user.details.account)
	item_count = Item.objects.active().at_location(location).this_account(request.user.details.account).count()
...
def customers(request):
	customers = Customer.objects.this_account(request.user.details.account).has_orders(request.GET.get('hasorders'))
	form = SearchForm(request.GET or None)
...
def customer_add(request):
	...
	if request.POST and form.is_valid():
		# ulozenie zakaznika
		new_customer = Customer.objects.create_customer(**form.cleaned_data, account=request.user.details.account)
...
def customer_orders(request, customer_id):
	customer = get_object_or_404(Customer, id=customer_id, account=request.user.details.account)
	orders = Order.objects.this_customer(customer).this_account(request.user.details.account).order_by('-start')
	form = SearchForm(request.GET or None)
...
def customer_remove(request, customer_id):
	customer = get_object_or_404(Customer, id=customer_id, account=request.user.details.account)
	order_count = Order.objects.this_customer(customer).this_account(request.user.details.account).count()
...
def addresses(request):
	if request.GET.get('city'):
		addresses = Address.objects.this_account(request.user.details.account).this_city(request.GET.get('city'))
	else:
		addresses = Address.objects.this_account(request.user.details.account)

	cities = Address.objects.this_account(request.user.details.account).values('city')
...
def address_add(request):
	...
	if request.POST and form.is_valid():
		...
		new_address = Address.objects.create_address(**form.cleaned_data, account=request.user.details.account)
...
def address_detail(request, address_id):
	...
	address = get_object_or_404(Address, id=address_id, account=request.user.details.account)
...
def address_edit(request, address_id):
	# nacitanie adresy podla id
	address = get_object_or_404(Address, id=address_id, account=request.user.details.account)
	form = AddressForm(request.POST or None, instance=address, account=request.user.details.account)
...
def employees(request):
	if request.GET.get('hasaddresses'):
		employees = Employee.objects.this_account(request.user.details.account).has_addresses(request.GET.get('hasaddresses'))
	else:
		employees = Employee.objects.this_account(request.user.details.account)
...
def employee_add(request):
	...
	if request.POST and form.is_valid():
		new_employee = Employee.objects.create_employee(**form.cleaned_data, account=request.user.details.account)
...
def employee_addresses(request, employee_id):
	employee = get_object_or_404(Employee, id=employee_id, account=request.user.details.account)
	addresses = Address.objects.this_employee(employee).this_account(request.user.details.account)
	form = SearchForm(request.GET or None)
